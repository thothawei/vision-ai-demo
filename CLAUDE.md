...
- **自動化測試腳本（`backend/test_all.sh`）**：把街景辨識與文件擷取（兩種模式）整合成一支腳本，遇到 Gemini 配額/過載錯誤會自動間隔重試。第一次執行結果：4 項測試只有 1 項（`street_test.jpg` 街景辨識）成功，另外 3 項在重試 6 次（等了 90 秒）後仍失敗。查證後發現原因：**當時我在腳本跑的同時手動下了額外的 curl 指令去檢查錯誤訊息內容**，這個手動請求跟腳本的自動重試搶用同一把 API key 的每分鐘 20 次配額，導致配額視窗一直被新請求刷新、清不掉。這不是腳本邏輯的 bug，是操作上的失誤（不該在自動化重試跑的時候手動戳同一個 API）。已修正：腳本開頭加註解提醒「執行期間不要手動呼叫 API」，並把重試次數從 6 次拉高到 10 次、間隔從 15 秒拉長到 20 秒，加大緩衝空間。
